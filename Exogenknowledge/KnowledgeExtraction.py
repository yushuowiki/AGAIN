    ...
    with open("Exogenknowledge/model_correction_output.txt", 'w') as file_w:
        with open("Exogenknowledge/model_output.txt", 'r') as file:
            ...
            for line_count in range(len(lines)):
                ...
                knowledge_fg = run_delecter(batch_size, n_attributes, labels)
                vector = torch.empty(0)
                dele_sign = 0
                w_z = torch.zeros((len(attr), 1))
                z = torch.zeros((len(attr), 1))
                m = torch.zeros((len(attr), 0))
                for i in range(1, len(attr)):
                    ...
                    has_k = 0
                z_c = 1 / (1 + torch.exp(Conti_conv_para * w_z * z))
                c_re = z_c*m + tensor_2d[line_count] * (1 - m)
                if dele_sign == 1:
...
def run_delecter(batch_size, n_attributes, labels):
    ...
    return batch_avg_cost, run_foln(foln_input)
