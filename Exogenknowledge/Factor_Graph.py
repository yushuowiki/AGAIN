...
if __name__ == '__main__':
    predicate = modelling.read_predicate('predicate.txt')
    formula = modelling.read_formula('formula.txt', predicate)
    database = modelling.read_data('data.txt', predicate)
    ...
    modelling.inference_str('Aa(x) ^ !Bb(x)  ^  !Cc(x) ^ !Dd(x) ^ Ee(x)  ^  Ff(x)  ^  Gg(x)', output, data, mln)
