...
def run_foln(foln_input):
    ...
    data, mln = modelling.model_config(predicate, formula, database, 'fp.mln', 'fp.db')
    output = modelling.activate_model(data, mln)
    output.tofile(os.getcwd() + '/Exogenknowledge/' + 'learnt_mln.mln')
    # modelling.inference_str(foln_input, output, data, mln)
    return modelling.inference_str(foln_input, output, data, mln)
