...
def LSM_eval(outputs,factor_graph_rules,batch_size, n_attributes, labels):
    LSM_num = 0
    new_tensor = torch.stack(outputs)
    for i in range(new_tensor.shape[1]):
        ...
        for j in range(new_tensor.shape[0]):
            ...
            for k in factor_graph_rules:
                if j == k["output_idx"] and k["class_idx"] == int(re.search(r'\((\d+)\)', str(labels[i])).group(1)) and k["logic"] != int(str(1 if np.array(new_tensor[j, i].detach().cpu()) > 0.5 else 0)): #
                    LSM_num = LSM_num+1
    return 1-(LSM_num/(n_attributes*batch_size))
...
def pgd_attack_random(model,model2, inputs_images,labels, attr_labels, eps=1, alpha=1, iters=40, randomize=True):
    ...
    for i in tqdm(range(inputs_images.shape[0]), desc="Perturbation sample generation", unit="batch"):
        ...
        label = labels[i]
        if randomize:
            delta = torch.rand_like(inputs_image, requires_grad=True).to(device)  # 生成随机数并创建与输入张量具有相同形状的张量的函数
...
if __name__ == '__main__':
    ...
    for eps_num in eps_list:
        ...
        for data_idx, data in enumerate(loader):
            ...
            end_time = time.time()
            CBM_test_time = end_time - start_time
            print("CBM Time", CBM_test_time)
            #print(outputs_Y)
            ...
            update_output, detect_num,factor_graph_rules = run_knowledge(batch_size, n_attributes, labels)
            ref_time_end = time.time()
            ref_cost = ref_time_end - ref_time
            print("ref time:",ref_cost)
            ...
            all_eval_LSM = all_eval_LSM + LSM_eval(adv_outputs, factor_graph_rules, batch_size, n_attributes, labels)
            all_eval_update_LSM = all_eval_update_LSM + LSM_eval(update_output,factor_graph_rules,batch_size, n_attributes, labels)
            ...
            if i > len(select_class)-1:
                break
        print('----------------------------------------')
        print('attack LSM: %.5f' % (all_eval_LSM / n_batch))
        print('correct LSM: %.5f' % (all_eval_update_LSM / n_batch))
    ...
    #     img.save('CBM/CUB_200_2011_adv_images/' + img_count + ".jpg")
    #     img_count = img_count + 1
    # img = Image.fromarray((delta + inputs_images).detach())  # If image_data is a NumPy array
    # img.save('CBM/CUB_200_2011_adv_images/' +i+".jpg")
