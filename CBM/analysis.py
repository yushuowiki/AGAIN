...
def accuracy_y(output, target):
    ...
    #print(max_indices)
    max_indices = max_indices.t().cpu()
    #temp = target.view(1, -1).expand_as(max_indices)
    correct = max_indices.eq(target)
    #print(correct)
    correct = correct.view(-1).float().sum(0, keepdim=True)
    #print(correct)
    return correct.mul_(100.0 / batch_size)

def top_k(output, target):
    """
    Computes the top k accuracy for multiple binary predictions
    output and target are Torch tensors
    """
    pred = output.cpu() >= 0.5
    acc = ((pred.int() == 1) & (target.int() == 1)).sum()
    acc = acc*100 / np.prod(np.array((target.int() == 1).size()))
    return acc
