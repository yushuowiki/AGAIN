"""
Train InceptionV3 Network using the CBM-200-2011 dataset
"""
import pdb
import os
import sys
...
from CBM.attrloss import logic_loss
from CBM.timestamps import time_stamps
...
from CBM import probe, tti, gen_cub_synthetic, hyperopt
from CBM.dataset import load_data, find_class_imbalance
from CBM.config import BASE_DIR, N_CLASSES, N_ATTRIBUTES, UPWEIGHT_RATIO, MIN_LR, LR_DECAY_SIZE
from CBM.models import ModelXtoCY, ModelXtoChat_ChatToY, ModelXtoY, ModelXtoC, ModelOracleCtoY, ModelXtoCtoY
...
def run_epoch(model, optimizer, loader, loss_meter, acc_meter, criterion, attr_criterion, args, is_training):
    ...
    for _, data in enumerate(loader):
        ...
        if is_training and args.use_aux:
            outputs, aux_outputs = model(inputs_var) # Total output and output of auxiliary classifier
            losses = []
            out_start = 0
        ...
        if attr_criterion is not None:
            if args.bottleneck:
                total_loss = sum(losses)/ args.n_attributes + logic_loss(torch.nn.Sigmoid()(torch.stack(outputs)), torch.nn.Sigmoid()(torch.stack(aux_outputs)), attr_labels_var)
...
def train(model, args):
    ...
    filename = time_stamps() + "log.txt"
    logger = Logger(os.path.join(args.log_dir, filename))
    ...
    if args.resume:
        checkpoint = torch.load(os.path.join(args.log_dir, 'best_model_%d.pth' % args.seed))
    ...
    for epoch in range(start_epochs, args.epochs):
        ...
        if best_val_acc < val_acc_meter.avg:
            ...
            state = {'model': model, 'optimizer': optimizer.state_dict(), 'epoch': best_val_epoch, 'best_val_acc': best_val_acc} # lyc
            torch.save(state, os.path.join(args.log_dir, 'best_model_%d.pth' % args.seed)) # lyc
...
def parse_arguments(experiment):
    ...
    if experiment == 'Probe':
        ...
        parser.add_argument('-resume', action='store_true',
                            help='Whether to continue train base on last time')
