...
from CBM.config import BASE_DIR, N_ATTRIBUTES
...
class CUBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(img_path) # LYC
        # print(type(img_data['class_label']))
        # print(img_path)
        ...
        try:
            ...
            img_path = '' + img_path # add CUB/ if in local and remove CUB/ in center
        ...
        # print('-------------------------------------------')
        # print(len(img_data['attribute_certainty']))
        # print(len(img_data['attribute_label']))
        if self.use_attr:
...
def load_data_several_class(select_class, pkl_paths, use_attr, no_img, batch_size, uncertain_label=False, n_class_attr=2, image_dir='images', resampling=False, resol=299):
    """
    Note: Inception needs (299,299,3) images with inputs scaled between -1 and 1
    Loads data with transformations applied, and upsample the minority class if there is class imbalance and weighted loss is not used
    NOTE: resampling is customized for first attribute only, so change sampler.py if necessary
    ...
    if resampling:
        sampler = BatchSampler(ImbalancedDatasetSampler(dataset), batch_size=batch_size, drop_last=drop_last)
        dataset = Filter_several_class(dataset, select_class)
        loader = DataLoader(dataset, batch_sampler=sampler)
    else:
        dataset = Filter_several_class(dataset, select_class)
        loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, drop_last=drop_last)
...
def Filter_several_class(dataset,select_class):
    dataset.data = [data_item for data_item in dataset.data if data_item['class_label'] in select_class]
    return dataset
