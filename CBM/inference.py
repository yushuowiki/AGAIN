"""
Evaluate trained models on the official CBM test set
"""
import os
import sys
import torch
...
from CBM.models import ModelXtoC
from CBM.dataset import load_data
from CBM.config import BASE_DIR, N_CLASSES, N_ATTRIBUTES
...
def eval(args):
    ...
    if args.model_dir:
        ...
        #model.load(torch.load(args.model_dir)['model']) # lyc Need to change, this time the saved model parameters are not the model
        #model = torch.load(args.model_dir)['model']
